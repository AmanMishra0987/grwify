  ...
  return (
    <footer className="border-t border-slate-900 bg-slate-950/60 py-12 px-4 lg:px-8 mt-auto">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <p className="text-sm text-slate-400 max-w-sm leading-relaxed">
            The next-generation virtual fitting room. Empowering shoppers to visualize clothing instantly, reducing returns and styling wardrobes with zero friction.
          </p>
        ...
        {/* Sitemap Quicklinks */}
        <div>
          <h4 className="text-xs font-semibold text-slate-300 uppercase tracking-widest mb-4">Workspace</h4>
          <ul className="space-y-2.5 text-sm">
            <li>
              <Link to="/select" className="text-slate-400 hover:text-white transition-colors">Product Grid</Link>
            </li>
            <li>
              <Link to="/upload" className="text-slate-400 hover:text-white transition-colors">Fitting Upload</Link>
            </li>
            <li>
              <Link to="/tryon" className="text-slate-400 hover:text-white transition-colors">Fitting Room</Link>
            </li>
          </ul>
        </div>
      ...
      <div className="max-w-7xl mx-auto border-t border-slate-900/60 mt-10 pt-6 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-4">
        <p>&copy; {new Date().getFullYear()} grwify Inc. All rights reserved.</p>
        <p className="flex items-center gap-1">
          Made with <Heart className="h-3 w-3 text-purple-500 fill-purple-500" /> for modern fashion.
        </p>
      </div>
    </footer>
  );
